// _verify_qa_nav.js — 问一问知识树导引四层下钻验证
const { spawn } = require('child_process');
const fs = require('fs'); const path = require('path');
const PORT = 9311;
const URL = 'http://127.0.0.1:8123/index.html';
const EDGE = 'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe';
const OUT = path.resolve(__dirname, '_tmp_qanav');
...
async function main() {
  ...
  ws.onmessage = m => { const d = JSON.parse(m.data); if (d.id && pend.has(d.id)) { pend.get(d.id)(d); pend.delete(d.id); } if (d.method === 'Runtime.exceptionThrown') errs.push((d.params.exceptionDetails.exception?.description || '').slice(0, 140)); };
  ...
  console.log('[1] 入口：搜索框 + 分叉图');
  console.log('   ', await ev(`JSON.stringify({
    hasSearch: !!document.getElementById('qaTreeSearch'),
    branches: [...document.querySelectorAll('.qt-branch')].map(function(b){
      return b.querySelector('.qt-name').textContent + '(' + b.querySelector('.qt-num').textContent.replace(' 张卡片','') + ')';
    }),
    filterBarVisible: $('qaFilterBar').style.display !== 'none'
  })`));
  await shot('qa_tree');

  console.log('\n[2] 点「阴阳能量」→ 系列目录');
  await ev(`qaNavOpen('阴阳能量'); 'ok'`);
  await sleep(900);
  console.log('   ', await ev(`JSON.stringify({
  ...
  await shot('qa_series');

  console.log('\n[3] 点第一个系列 → 卡片清单');
  await ev(`qaNavSeries(qaSeriesKey(qaGroupCards('阴阳能量')[0].kpId)); 'ok'`);
  await sleep(900);
  console.log('   ', await ev(`JSON.stringify({
    crumbs: [...document.querySelectorAll('.qc-item')].map(function(x){return x.textContent}),
    count: document.querySelectorAll('.qtitle-item').length,
    first3: [...document.querySelectorAll('.qti-t')].slice(0,3).map(function(x){return x.textContent.slice(0,26)})
  })`));
  await shot('qa_titles');
  ...
  console.log('   ', await ev(`JSON.stringify({
    ...
    hasPrev: document.querySelectorAll('.qsn-btn')[0].disabled === false,
    hasNext: document.querySelectorAll('.qsn-btn')[1].disabled === false,
    evBtns: document.querySelectorAll('.qa-single .ev-go').length
  })`));
  await shot('qa_single');

  console.log('\n[5] 翻到下一张');
  await ev(`qaNavStep(1); 'ok'`);
  await sleep(700);
  console.log('   ', await ev(`JSON.stringify({ idx: QA_NAV.idx, pos: (document.querySelector('.qsn-pos')||{}).textContent })`));
  ...
  console.log('   ', await ev(`JSON.stringify({
    ...
    items: [...document.querySelectorAll('.qts-item')].slice(0,4).map(function(x){return x.textContent.slice(0,26)})
  })`));
  await shot('qa_search');

  console.log('\n[7] 点搜索结果 → 直达单卡');
  await ev(`(()=>{var it=document.querySelector('.qts-item'); if(it){it.click(); return 'clicked';} return 'none';})()`);
  await sleep(900);
  console.log('   ', await ev(`JSON.stringify({ level: QA_NAV.level, pos:(document.querySelector('.qsn-pos')||{}).textContent })`));
