...
for (const c of cards) {
  ...
  for (let i = 0; i < c.evidence.length; i++) {
    ...
    if (!best || best.L < 8) { weak++; rows.push({ qa: c.qaId, i, kind: 'WEAK', declBv, declT, L: best ? best.L : 0, txt: (e.text || '').slice(0, 36) }); continue; }
    const inDecl = corpus[declBv] ? Math.max(...corpus[declBv].map(s => lcs(q, norm(s.text)))) : 0;
    if (best.bv !== declBv && inDecl < best.L * 0.8) {
      cross++; rows.push({ qa: c.qaId, i, kind: 'CROSS_BV', declBv, declT, bestBv: best.bv, bestSec: best.sec, bestTs: fmt(best.sec), L: best.L, inDecl, txt: (e.text || '').slice(0, 36), hit: best.text.slice(0, 36) });
    } else if (Math.abs(best.sec - declT) > 8) {
      offset++; rows.push({ qa: c.qaId, i, kind: 'OFFSET', declBv, declT, bestBv: best.bv, bestSec: best.sec, bestTs: fmt(best.sec), L: best.L, txt: (e.text || '').slice(0, 36), hit: best.text.slice(0, 36) });
    } else ok++;
  }
}
console.log('\n=== 全量 evidence 全局锚定 ===');
...
fs.writeFileSync(path.join(__dirname, '_global_anchor.json'), JSON.stringify({ stat: { n, ok, cross, offset, weak }, rows }, null, 1), 'utf-8');
console.log('\n完整清单: scripts/_global_anchor.json');
